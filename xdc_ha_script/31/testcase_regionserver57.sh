...
{       echo ">>>>>>>>compare data of table named xdc_test now !!!!!!" 2>&1 | tee -a ./stop_regionserverpassive.log
        ...
        echo ">>>>>>>>compare data complete now " 2>&1 | tee -a ./stop_regionserverpassive.log
        if [ $rt -le 2 ]; then
                echo "The data in xdc_test on both DCs is the same ........." 2>&1 | tee -a ./stop_regionserverpassive.log
                return 1
        else
                echo "The data in xdc_test on both DCs is not the same ........." 2>&1 | tee -a ./stop_regionserverpassive.log
                return 0
        fi
}

#load data
nohup sh ./test_workload.sh 2>&1 | tee -a ./stop_regionserverpassive.log &
sleep 300 2>&1 | tee -a ./stop_regionserver.log

#stop region server of 57
echo "stop region server of passive DC 57" 2>&1 | tee -a ./stop_regionserver.log
...
sleep 480
#start regionserver as start HDFS
echo "start regionserver now !!!!!" 2>&1 | tee -a ./stop_regionserver.log
curl -u admin:admin -H "X-Requested-By: ambari" -X PUT -d '{"RequestInfo":{"context":"Start Service"},"Body":{"ServiceInfo":{"state":"STARTED"}}}' 10.10.23.57:8080/api/v1/clusters/ambari/services/HBASE 2>&1 | tee -a ./stop_regionserver.log
